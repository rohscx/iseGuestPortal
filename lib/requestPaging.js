...
module.exports = async function (data) {
    return new Promise((resolve,reject) => {
        ...
        if (!resourcesExist) return resolve({cache:resources,name:data.authData.name});
        if (!nextPage) return resolve({cache:resources,name:data.authData.name});
          ...
          [Symbol.iterator]: function (){
            ...
            return {
              next: async function (){
                ...
                const request = await asyncRequest(generateOptions(server, auth, nextPage));
        ...
        ;( async function () {
          ...
          while (!iteratorData.done) {
            ...
            if (moreIteratorData.done) return resolve({cache,name})
          }
